...
<details>
  ...
  <details>
    ...
    1. 이용자가 각 상품의 이름과 가격만 듣고 무엇을 명시하는지 알 수 없으므로 dl 요소의 dd 태그를 사용하여 각각 '상품명', '정상가', '할인가'임을 명시  
    aria-hidden="true" 속성을 이용하여 스크린리더에서 읽을 수 있도록 함  
      
    
    2. 불필요한 '[', ']' 기호나 상품 이미지의 정보가 반복되어 혼란을 줄 수 있으므로 aria-hidden="true" 속성을 이용하여 스크린리더에서 읽을 수 없도록 함

    3. 링크 영역에는 target 속성을 부여하여 상세페이지 이동 링크임을 알 수 있도록 함
